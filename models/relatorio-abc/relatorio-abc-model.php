...
class RelatorioAbcModel extends MainModel
{
	...
	public function consultar() {
		...
		$query = $this->db->query(
			'SELECT round(sum(componente_quantidade * componente_valor_unitario) ,2) as custo_atividade, custo_descricao, direcionador_descricao, atividade_direcionador_quantidade,  round(sum(componente_quantidade * componente_valor_unitario)/atividade_direcionador_quantidade ,2) as custo_direcionador FROM componente, tipo, produtividade, custo, atividade, direcionador WHERE componente_fk_tipo = ?  AND tipo_id = ? AND produtividade_fk_tipo = ? AND componente_fk_custo = custo_id AND atividade_fk_custo = custo_id AND atividade_fk_direcionador = direcionador_id group by componente_fk_custo ', 
			array(
				$tipo, $tipo, $tipo
			)
		);
	...
	public function paginacao () {
		...
		*/
		...
		$caminho_relatorio_abc = HOME_URI . '/relatorio-custo/index/page/';
		...
		if ( $current <= ( $last - $offset1 ) ) {
			echo " ... <a href='$caminho_relatorio_abc$last'>$last</a>";
		}
	} // paginacao
