...
class RelatorioCustoUnitarioModel extends MainModel
{
	...
	public function consultar() {
		...
		$query = $this->db->query(
			'SELECT tipo_descricao, round(produtividade_valor,2) as produtividade, round(sum(componente_quantidade * componente_valor_unitario),2) as total,   round(sum(componente_quantidade * componente_valor_unitario)/produtividade_valor,2) as unitario, unidade_descricao as unidade FROM componente, tipo, produtividade, unidade WHERE componente_fk_tipo = ?  AND tipo_id = ? AND produtividade_fk_tipo = ?  AND unidade_id = produtividade_fk_unidade', 
			array(
				$tipo, $tipo, $tipo
			)
		);
