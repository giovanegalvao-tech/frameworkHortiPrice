...
class RelatorioCustoModel extends MainModel
{
	...
	public function consultar() {
		...
		$query = $this->db->query(
			'SELECT componente_descricao, componente_quantidade, round(componente_valor_unitario,2) as componente_valor_unitario, round((componente_quantidade * componente_valor_unitario),2) as total, unidade_descricao FROM componente, tipo, produtividade, unidade WHERE componente_fk_tipo = ?  AND tipo_id = ? AND produtividade_fk_tipo = ? AND componente_fk_unidade = unidade_id ', 
			array(
				$tipo, $tipo, $tipo
			)
		);
