<?php 
/**
 * Classe para gerenciar subcategoria
 */

error_reporting(0);

class SubcategoriaModel extends MainModel
{
	...
	public function consultar_subcategoria () {
		...
		// Verifica se um parâmetro foi enviado para carregar uma subcategoria
		if ( is_numeric( chk_array( $this->parametros, 0 ) ) ) {
			
			// Configura o ID para enviar para a subcategoria
			$id = array ( chk_array( $this->parametros, 0 ) );
			
			// Configura a cláusula where da consulta
			$where = " WHERE subcategoria_id = ? ";
		}
		...
		// Faz a consulta
		$query = $this->db->query(
			'SELECT * FROM subcategoria ' . $where . ' ORDER BY subcategoria_id DESC' . $query_limit,
			$id
		);
	...
	public function editar_subcategoria () {
		...
		$subcategoria_id = chk_array( $this->parametros, 1 );
  ...
  background-color: #ef5350; color: white;">Atenção! Para efetuar a ação é necessário informar todos os campos do formulário.</div>';
		...
		incluir_subcategoria.
		
		Se verdadeiro, atualiza os dados conforme a requisição.
		*/
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['incluir_subcategoria'] ) ) {
		
			// Remove o campo incluir_subcategoria para não gerar problema com o PDO
			unset($_POST['incluir_subcategoria']);
			
			// Verifica se a descrição foi enviada
			$descricao = chk_array( $_POST, 'subcategoria_descricao' );
						
			// Adiciona a descricao no $_POST		
			$_POST['subcategoria_descricao'] = $descricao;
			
			
			// Atualiza os dados
			$query = $this->db->update('subcategoria', 'subcategoria_id', $subcategoria_id, $_POST);
			
			// Verifica a consulta
			if ( $query ) {
				echo '<meta http-equiv="Refresh" content="0; url=' . HOME_URI . '/subcategoria/">';
				echo '<script type="text/javascript">window.location.href = "' . HOME_URI . '/subcategoria/";</script>';
			}
		...
		$query = $this->db->query(
			'SELECT * FROM subcategoria WHERE subcategoria_id = ? LIMIT 1',
			array( $subcategoria_id )
		);
	...
	public function incluir_subcategoria() {
		...
		*/
		...
		$query = $this->db->insert( 'subcategoria', $_POST );
		
		// Verifica a consulta
		if ( $query ) {
		
			// Retorna uma mensagem
			$this->form_msg = '<div class="z-depth-3" style="border-color: #0288d1; background-color: #1b5e20; color: white;">Subcategoria cadastrada com sucesso!</div>';
  ...
  background-color: #ef5350; color: white;">Atenção! Para efetuar a ação é necessário informar todos os campos do formulário.</div>';
	...
	public function excluir_subcategoria () {
		...
		if ( chk_array( $this->parametros, 2 ) != 'confirma' ) {
			...
			$mensagem  = '<div class="z-depth-3" style="border-color: #ef6c00; background-color: #1b5e20;
  			color: white;">Tem certeza que deseja apagar a subcategoria(ID: '. $this->parametros[1].')?</div>';
			$mensagem .= '<p><a class="waves-effect waves-light btn green darken-4" href="' . $_SERVER['REQUEST_URI'] . '/confirma/">Sim</a> | ';
			$mensagem .= '<a class="waves-effect waves-light btn red darken-4" href="' . HOME_URI . '/subcategoria/">Não</a></p>';
			// Retorna a mensagem e não excluir
			return $mensagem;
		}
		
		// Configura o ID da subcategoria
		$subcategoria_id = (int)chk_array( $this->parametros, 1 );
		
		// Executa a consulta
		$query = $this->db->delete( 'subcategoria', 'subcategoria_id', $subcategoria_id );
	...
	public function paginacao () {
		...
		*/
		...
		$caminho_subcategoria = HOME_URI . '/subcategoria/index/page/';
		...
		if ( $current <= ( $last - $offset1 ) ) {
			echo " ... <a href='$caminho_subcategoria$last'>$last</a>";
		}
	} // paginacao
