<?php 
/**
 * Classe para gerenciar custo variável
 */

class CustoVariavelModel extends MetodoCusteio
{
	...
	public function consultar_custovariavel () {
		...
		// Faz a consulta
		$query = $this->db->query(
			'SELECT custovariavel_id, tipo.tipo_descricao, produtividade.produtividade_mes, produtividade.produtividade_ano, custovariavel_precovenda, custovariavel_margem FROM custovariavel, tipo, produtividade WHERE custovariavel_fk_tipo = tipo.tipo_id and custovariavel_fk_produtividade = produtividade.produtividade_id' . ' ORDER BY custovariavel_id DESC' . $query_limit,
			$id
		);
	...
	public function calcular(){
		...
		$query = $this->db->insert( 'custovariavel', array(
				'custovariavel_fk_tipo' => chk_array( $this->form_data, 'custovariavel_fk_tipo' ),
				'custovariavel_fk_produtividade' => chk_array($this->form_data, 'custovariavel_fk_produtividade'),
				'custovariavel_precovenda' => $this->calcularPrecoVenda(chk_array( $this->form_data, 'custovariavel_fk_tipo' ), chk_array($this->form_data, 'custeiovariavel_fk_produtividade'), chk_array( $this->form_data, 'custovariavel_margem')),
				'custovariavel_margem' => chk_array( $this->form_data, 'custovariavel_margem' )
	...
	public function calcularPrecoVenda($tipo, $produtividade, $valor){
		...
		$query = $this->db->query(
			'SELECT round(sum(componente_quantidade*componente_valor_unitario)/produtividade.produtividade_valor,2) as resultado FROM componente, tipo, produtividade WHERE componente_fk_tipo = ?  AND tipo_id = ? AND produtividade_fk_tipo = ? AND ( componente_fk_classificacao = 1 OR componente_fk_classificacao = 3 OR componente_fk_classificacao = 4 )', 
			array(
				$tipo, $tipo, $tipo
			)
		);
		...
		foreach ($lista as $fetch_custovariaveldata):
			$custoUnitario =  $fetch_custovariaveldata['resultado'];
		 endforeach;
	...
	public function excluir_custovariavel () {
		...
		// Configura o ID da custovariavel
		$custovariavel_id = (int)chk_array( $this->parametros, 1 );
		
		// Executa a consulta
		$query = $this->db->delete( 'custovariavel', 'custovariavel_id', $custovariavel_id );
		
		// Redireciona para a página de administração de notícias
		echo '<meta http-equiv="Refresh" content="0; url=' . HOME_URI . '/custo-variavel/">';
		echo '<script type="text/javascript">window.location.href = "' . HOME_URI . '/custo-variavel/";</script>';
	...
	public function paginacao () {
		...
		*/
		...
		$query = $this->db->query(
			'SELECT COUNT(*) as total FROM custovariavel '
		);
		$total = $query->fetch();
		$total = $total['total'];
		
		// Configura o caminho para a paginação
		$caminho_custovariavel = HOME_URI . '/custo-variavel/index/page/';
		...
		if ( $current <= ( $last - $offset1 ) ) {
			echo " ... <a href='$caminho_custovariavel$last'>$last</a>";
		}
	} // paginacao		
