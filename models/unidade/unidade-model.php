<?php 
/**
 * Classe para gerenciar unidade
 */

error_reporting(0);

class UnidadeModel extends MainModel
{
	...
	public function consultar_unidade () {
	
		// Configura as variáveis que vamos utilizar
		$id = $where = $query_limit = null;
		
		// Verifica se um parâmetro foi enviado para carregar uma unidade
		if ( is_numeric( chk_array( $this->parametros, 0 ) ) ) {
			
			// Configura o ID para enviar para a unidade
			$id = array ( chk_array( $this->parametros, 0 ) );
			
			// Configura a cláusula where da consulta
			$where = " WHERE unidade_id = ? ";
		}
		...
		// Faz a consulta
		$query = $this->db->query(
			'SELECT * FROM unidade ' . $where . ' ORDER BY unidade_id DESC' . $query_limit,
			$id
		);
	...
	public function editar_unidade () {
  ...
  background-color: #ef5350; color: white;">Atenção! Para efetuar a ação é necessário informar todos os campos do formulário.</div>';
		...
		*/
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['incluir_unidade'] ) ) {
		
			// Remove o campo incluir_unidade para não gerar problema com o PDO
			unset($_POST['incluir_unidade']);
			
			// Verifica se a descrição foi enviada
			$descricao = chk_array( $_POST, 'unidade_descricao' );
						
			// Adiciona a descricao no $_POST		
			$_POST['unidade_descricao'] = $descricao;
			
			
			// Atualiza os dados
			$query = $this->db->update('unidade', 'unidade_id', $unidade_id, $_POST);
			
			// Verifica a consulta
			if ( $query ) {
				echo '<meta http-equiv="Refresh" content="0; url=' . HOME_URI . '/unidade/">';
				echo '<script type="text/javascript">window.location.href = "' . HOME_URI . '/unidade/";</script>';
			}
		...
		$query = $this->db->query(
			'SELECT * FROM unidade WHERE unidade_id = ? LIMIT 1',
			array( $unidade_id )
		);
	...
	public function incluir_unidade() {
		...
		*/
		...
		$query = $this->db->insert( 'unidade', $_POST );
		
		// Verifica a consulta
		if ( $query ) {
		
			// Retorna uma mensagem
			$this->form_msg = '<div class="z-depth-3" style="border-color: #0288d1; background-color: #1b5e20; color: white;">Unidade cadastrada com sucesso!</div>';
  ...
  background-color: #ef5350; color: white;">Atenção! Para efetuar a ação é necessário informar todos os campos do formulário.</div>';
	...
	public function excluir_unidade () {
		...
		if ( chk_array( $this->parametros, 2 ) != 'confirma' ) {
			...
			$mensagem  = '<div class="z-depth-3" style="border-color: #ef6c00; background-color: #1b5e20;
  			color: white;">Tem certeza que deseja apagar a unidade(ID: '. $this->parametros[1].')?</div>';
			$mensagem .= '<p><a class="waves-effect waves-light btn green darken-4" href="' . $_SERVER['REQUEST_URI'] . '/confirma/">Sim</a> | ';
			$mensagem .= '<a class="waves-effect waves-light btn red darken-4" href="' . HOME_URI . '/unidade/">Não</a></p>';
		...
		// Configura o ID da unidade
		$unidade_id = (int)chk_array( $this->parametros, 1 );
		
		// Executa a consulta
		$query = $this->db->delete( 'unidade', 'unidade_id', $unidade_id );
	...
	public function paginacao () {
		...
		*/
		...
		$query = $this->db->query(
			'SELECT COUNT(*) as total FROM unidade '
		);
		$total = $query->fetch();
		$total = $total['total'];
		
		// Configura o caminho para a paginação
		$caminho_unidade = HOME_URI . '/unidade/index/page/';
		...
		if ( $current <= ( $last - $offset1 ) ) {
			echo " ... <a href='$caminho_unidade$last'>$last</a>";
		}
	} // paginacao
