<?php 
/**
 * Classe para gerenciar custo
 */
error_reporting(0);

class CustoModel extends MainModel
{
	...
	public function consultar_custo () {
	
		// Configura as variáveis que vamos utilizar
		$id = $where = $query_limit = null;
		
		// Verifica se um parâmetro foi enviado para carregar uma custo
		if ( is_numeric( chk_array( $this->parametros, 0 ) ) ) {
			
			// Configura o ID para enviar para a custo
			$id = array ( chk_array( $this->parametros, 0 ) );
			
			// Configura a cláusula where da consulta
			$where = " WHERE custo_id = ? ";
		}
		...
		// Faz a consulta
		$query = $this->db->query(
			'SELECT * FROM custo ' . $where . ' ORDER BY custo_id DESC' . $query_limit,
			$id
		);
	...
	public function editar_custo () {
		...
		// Configura o ID da custo
		$custo_id = chk_array( $this->parametros, 1 );
  ...
  background-color: #ef5350; color: white;">Atenção! Para efetuar a ação é necessário informar todos os campos do formulário.</div>';
		...
		*/
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['incluir_custo'] ) ) {
		
			// Remove o campo incluir_custo para não gerar problema com o PDO
			unset($_POST['incluir_custo']);
			
			// Verifica se a descrição foi enviada
			$descricao = chk_array( $_POST, 'custo_descricao' );
						
			// Adiciona a descricao no $_POST		
			$_POST['custo_descricao'] = $descricao;
			
			
			// Atualiza os dados
			$query = $this->db->update('custo', 'custo_id', $custo_id, $_POST);
			
			// Verifica a consulta
			if ( $query ) {
				echo '<meta http-equiv="Refresh" content="0; url=' . HOME_URI . '/custo/">';
				echo '<script type="text/javascript">window.location.href = "' . HOME_URI . '/custo/";</script>';
		...
		$query = $this->db->query(
			'SELECT * FROM custo WHERE custo_id = ? LIMIT 1',
			array( $custo_id )
		);
	...
	public function incluir_custo() {
		...
		*/
		...
		$query = $this->db->insert( 'custo', $_POST );
		
		// Verifica a consulta
		if ( $query ) {
		
			// Retorna uma mensagem
			$this->form_msg = '<div class="z-depth-3" style="border-color: #0288d1; background-color: #1b5e20; color: white;">Custo cadastrado com sucesso!</div>';
  ...
  background-color: #ef5350; color: white;">Atenção! Para efetuar a ação é necessário informar todos os campos do formulário.</div>';
	...
	public function excluir_custo () {
		...
		if ( chk_array( $this->parametros, 2 ) != 'confirma' ) {
			...
			$mensagem  = '<div class="z-depth-3" style="border-color: #ef6c00; background-color: #1b5e20;
  			color: white;">Tem certeza que deseja apagar o custo(ID: '. $this->parametros[1].')?</div>';
			$mensagem .= '<p><a class="waves-effect waves-light btn green darken-4" href="' . $_SERVER['REQUEST_URI'] . '/confirma/">Sim</a> | ';
			$mensagem .= '<a class="waves-effect waves-light btn red darken-4" href="' . HOME_URI . '/custo/">Não</a></p>';
		...
		// Configura o ID da custo
		$custo_id = (int)chk_array( $this->parametros, 1 );
		
		// Executa a consulta
		$query = $this->db->delete( 'custo', 'custo_id', $custo_id );
	...
	public function paginacao () {
		...
		*/
		...
		$query = $this->db->query(
			'SELECT COUNT(*) as total FROM custo '
		);
		$total = $query->fetch();
		$total = $total['total'];
		
		// Configura o caminho para a paginação
		$caminho_custo = HOME_URI . '/custo/index/page/';
		...
		if ( $current <= ( $last - $offset1 ) ) {
			echo " ... <a href='$caminho_custo$last'>$last</a>";
		}
	} // paginacao
